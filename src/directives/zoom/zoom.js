  ...
  Vue.directive('sinotruk-zoom', {
    inserted(el, binding) {
      ...
      zoom.on('zoom', (event) => {
        ...
        if (event.sourceEvent) {
          const movedX =
            event.sourceEvent.clientX - zoomDirective[el.id].start.clientX;
          const movedY =
            event.sourceEvent.clientY - zoomDirective[el.id].start.clientY;
          ...
          target.attr(
            'style',
            `transform:scale(${event.transform.k}) 
