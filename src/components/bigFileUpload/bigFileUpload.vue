<template>
  <div class="upload-container">
    <el-upload
      ...
      class="inputField"
      :disabled="disabled">
        <el-button size="mini" slot="trigger" class="header" v-if="!disabled">选择文件</el-button>
        <el-button size="mini" @click="uploadToServer" class="header" v-if="!disabled">上传</el-button>
        <span v-if="!disabled" v-show="timeShow" class="header">剩余时间：{{ reTime }}  s</span>
        <ul class="file-list">
          <li v-for="file in fileList" :key="file.uid">
            ...
            <div >
              <el-progress
                ...
                <i class="el-icon-circle-check" style="color: green" v-if="iconShow(file, 'finished')"></i>
                <i class="el-icon-warning-outline" style="color: red" v-if="iconShow(file, 'warning')"></i>
            </div>
            <div class="operation" >
              <el-button v-if="!disabled" size="mini" circle icon="el-icon-video-pause" v-show="iconShow(file, 'pause')" @click="file.isBreak = true"></el-button>
              <el-button v-if="!disabled" size="mini" circle icon="el-icon-video-play" v-show="iconShow(file, 'containue')" @click="reUpload(file.uid)"></el-button>
              <el-button size="mini" circle icon="el-icon-download" v-show="iconShow(file, 'download')" @click="downloadFile(file.fileUploadId)"></el-button>
              <el-button v-if="!disabled" size="mini" circle icon="el-icon-close" v-show="iconShow(file, 'close')" @click="deleteFile(file.uid)"></el-button>
              <el-button v-if="!disabled" size="mini" circle icon="el-icon-refresh-left" v-show="iconShow(file, 'reupload')" @click="reUpload(file.uid)"></el-button>
            </div>
          </li>
        </ul>
    </el-upload>
...
import { isExist, bigFileUpload, removeBigFileByMd5, fileDownload } from './api'
...
export default {
      ...
      handler(arr) {
        if (arr && arr.length != 0) {
          arr.map(ele => {
            ...
            // 防止重复添加
            const flag = this.fileList.some(ele => ele.uid == item.uid)
            if (!flag) {
  ...
  methods: {
    async beforeUpload(file) {
      ...
      // md5传给后端判断是否已上传
      const res = await isExist({ md5: md5 })
      const ele = this.fileList.find(ele => ele.uid == file.uid)
      if (ele) {
        ele.md5 = md5
    ...
    async submitUpload(options) {
      ...
      // 串行Promise
      const uploadRes = await chunks.reduce(async(previourPromise, chunk, currentIndex) => {
        ...
        return new Promise((resolve) => {
          ...
          if (currentIndex < truncation) {
            currFile.time = Math.ceil((1 - (currentIndex + 1) / sliceCount) * 50)
            currFile.progress.percentage = Math.ceil(((currentIndex + 1) / sliceCount) * 100)
            const info = {
              status: 'success',
          ...
          bigFileUpload(formData).then(res => {
            ...
            this.fileList.map(ele => {
              if (ele.time) { time += parseFloat(ele.time) }
            })
    ...
    handleChange(file) {
      const isExist = this.fileList.some(ele => ele.uid == file.uid)
      if (!isExist) {
    ...
    reUpload(uid) {
      const ele = this.fileList.find(ele => ele.uid == uid)
      ele.status = 'ready'
      ele.isBreak = false
    ...
    async deleteFile(uid) {
      ...
      // 在fileList中删除文件
      const index = this.fileList.findIndex(ele => ele.uid == uid)
      this.fileList.splice(index, 1)
      // 在el-upload中删除文件
      const elIndex = this.$refs.upload.uploadFiles.findIndex(ele => ele.uid == uid)
