  ...
  <transition name="el-zoom-in-top">
    <div
      ...
      <div class="panel-footer">
        ...
        <el-button size="mini" type="primary" @click="handleConfirm()" :disabled="!isValidValue([minDate, maxDate])">确认</el-button>
      </div>
    </div>
  </transition>
</template>
...
import { limitTimeRange, modifyTime, timeWithinRange, clearMilliseconds } from '../utils/date-util'
export default {
  ...
  methods: {
    ...
    handleChange(){
      ...
      if(this.isValidValue([this.minDate, this.maxDate])){
        this.$refs.startSpinner.selectableRange = [modifyTime(this.minDate, 0, 0, 0), this.maxDate]
        this.$refs.endSpinner.selectableRange = [this.minDate, modifyTime(this.maxDate, 23, step, 0)]
  ...
  mounted(){
    this.minDate = modifyTime(new Date(), 0, 0, 0)
    this.maxDate = modifyTime(new Date(), 0, 0, 0)
    this.oldValue = [modifyTime(new Date(), 0, 0, 0), modifyTime(new Date(), 0, 0, 0)]
    const date = new Date()
    const step = parseInt(this.interval.step.split(':')[1])
    this.$refs.startSpinner.selectableRange = [new Date(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0), new Date(this.maxDate)]
    this.$refs.endSpinner.selectableRange = [new Date(this.minDate), new Date(date.getFullYear(), date.getMonth(), date.getDate(), 23, step, 0)]
  }
