<template>
  <transition name="el-zoom-in-toop">
    <div v-show="visible" class="date-panel">
      ...
      <date-spinner ref="spinner" :date="date" @change="handleChange">
      </date-spinner>
      <div class="panel-footer">
        ...
        <el-button
          ...
          type="primary"
          @click="handleConfirm()"
          :disabled="!isValidValue(this.date)"
          >确认</el-button
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    isValidValue(value) {
      const min = this.selectableRange[0]
        ? this.selectableRange[0].getTime()
        : Number.NEGATIVE_INFINITY;
      const max = this.selectableRange[1]
        ? this.selectableRange[1].getTime()
        : Number.POSITIVE_INFINITY;
      return (
        value &&
        !isNaN(value) &&
        new Date(value).getTime() >= min &&
