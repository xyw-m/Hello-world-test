  ...
  <div class="time-select-spinner">
    <el-scrollbar
      ...
      <li
        ...
        :class="{'active': timesLabelList[index] === currentTime, 'disabled': disabled}"
        @click="handleClick({value: index, disabled: disabled})">{{ timesLabelList[index] }}</li>
    </el-scrollbar>
  </div>
</template>
<script>
...
const getIntervalTimes = function(ranges, start, end, step){
  ...
  if(start && end && step){
    ...
    while(compareTime(current, end) <= 0){
      const disabled = compareTime(current,  minTime) < 0 || 
                        compareTime(current, maxTime) > 0
...
export default {
  ...
  computed: {
    timesList(){
      return getIntervalTimes(this.formattedranges, this.interval.start, this.interval.end, this.interval.step).timesList
    },
    timesLabelList(){
      return getIntervalTimes(this.formattedranges, this.interval.start, this.interval.end, this.interval.step).timesLabelList
    },
    currentTime(){
    ...
    formattedranges(){
      ...
      if(min instanceof Date && max instanceof Date){
        const formattedMin = formatTime({hours: min.getHours(), minutes: min.getMinutes()})
        const formattedMax = formatTime({hours: max.getHours(), minutes: max.getMinutes()})
  ...
  methods: {
    ...
    handleScroll(e){
      let max = this.timesList.length
      let value = Math.min(Math.round((this.$refs.time.wrap.scrollTop - (this.scrollBarHeight() * 0.5 - 10) / this.typeItemHeight() + 3) / this.typeItemHeight()), max);
